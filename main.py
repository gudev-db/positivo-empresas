...
st.set_page_config(
    layout="wide",
    page_title="Agente Positivo Empresas",
    page_icon="assets/page-icon.png"
)
st.image('assets/macLogo.png', width=300)

st.header('Agente Positivo Empresas')
st.header(' ')
...
db_briefings = banco["briefings_Positivo_Empresas"]  
...
tab_chatbot, tab_aprovacao, tab_geracao, tab_briefing, tab_briefing_gerados, tab_resumo = st.tabs([
    "💬 Chatbot Positivo Empresas", 
    "✅ Aprovação de Conteúdo", 
    "✨ Geração de Conteúdo",
    "📋 Geração de Briefing Positivo Empresas",  
    "📋 Briefings Gerados",
    "📝 Resumo de Textos",
])
...
with tab_chatbot:  
    st.header("Chat Virtual Positivo Empresas")
    st.caption("Pergunte qualquer coisa sobre as diretrizes e informações da Positivo Empresas")
    ...
    if prompt := st.chat_input("Como posso ajudar?"):
        ...
        contexto = f"""
        ###BEGIN SYSTEM PROMPT####
        Você é um assistente virtual especializado na Positivo Empresas
        Baseie todas as suas respostas nas diretrizes oficiais da Positivo Empresas
    ...
    with col1:
        ...
        if st.button("Gerar Especificações", key="gen_visual"):
            with st.spinner('Criando guia de estilo...'):
                prompt = f"""
                Você é um designer que trabalha para a Macfor Marketing digital e você deve gerar conteúdo criativo para o cliente Positivo Empresas.
...
with tab_briefing:
    st.header("Gerador de Briefing Positivo Empresas")
    st.caption("Crie briefings completos para diferentes áreas de atuação da Positivo Empresas")
    
    # Conexão com MongoDB para briefings
    db_briefings = client2['briefings_Positivo_Empresas']
    ...
    with tab_new:
        ...
        def criar_campo_selecionavel(rotulo, tipo="text_area", opcoes=None, padrao=None, key_suffix=""):
            ...
            if auto_preencher:
                ...
                prompt = f"Com base no seguinte contexto:\n{contexto}\n\n E o objetivo do briefing {objetivo_geral} \n\nPreencha o campo '{rotulo}' para um briefing do tipo {tipo_briefing} no Positivo Empresas. Retorne APENAS o valor para o campo, sem comentários ou formatação adicional."
        ...
        if st.button("🔄 Gerar Briefing Completo", type="primary"):
            with st.spinner('Construindo briefing profissional...'):
                try:
                    ...
                    prompt_parts = [
                        f"# BRIEFING {tipo_briefing.upper()} - Positivo_Empresas",
    ...
    with tab_saved:
        ...
        collection_briefings = client2.briefings_Positivo_Empresas.briefings  # Ajuste aqui
...
with tab_resumo:
    ...
    st.caption("Resuma textos longos mantendo o alinhamento com as diretrizes da Positivo Empresas")
            ...
            if not texto_original.strip():
                ...
                with st.spinner("Processando resumo..."):
                    try:
                        ...
                        prompt = f"""
                        Crie um resumo profissional deste texto para a Positivo Empresas,
                        ...
                        - Mantenha o tom profissional da Positivo Empresas
                        ...
                        st.download_button(
                            "📋 Copiar Resumo",
                            data=resposta.text,
                            file_name="resumo_Positivo_Empresas.txt",
                            mime="text/plain"
                        )
    ...
    with tab_briefing_gerados:
        st.header("📚 Briefings Gerados - Positivo_Empresas")
        st.markdown("---")
